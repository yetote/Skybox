...
public class TextureHelper {
    ...
    public static int loadCubeMap(Context context, int[] cubeRecourse) {
        final int[] textureObjectIds = new int[1];
        glGenTextures(1, textureObjectIds, 0);
        if (textureObjectIds[0] == 0) {
            Log.e(TAG, "loadCubeMap: " + "无法加载天空盒纹理文件");
            return 0;
        }
        final BitmapFactory.Options options = new BitmapFactory.Options();
        options.inScaled = false;
        ...
        for (int i = 0; i < 6; i++) {
            cubeBitmaps[i] = BitmapFactory.decodeResource(context.getResources(), cubeRecourse[i], options);
            if (cubeBitmaps[i] == null) {
                Log.e(TAG, "loadCubeMap: " + "无法加载" + cubeRecourse[i] + "资源文件");
                glDeleteTextures(1, textureObjectIds, 0);
                return 0;
            }
        }
